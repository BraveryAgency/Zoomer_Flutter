...
import 'package:zoomer/utils/open_vidu/request_config.dart';
...
class ApiClient {
  ...
  Future<T> request<T>(Config config, {bool autoLogin = false}) async {
    ...
    if (_response.statusCode == HttpStatus.ok) {
      if (config.hasResponse) {
        return Future<T>.value(config.responseType?.parse(_response));
      } else {
  ...
  Future<dynamic> _processError(HttpClientResponse response, Config config,
      {Future<dynamic> Function()? onAutoLoginSuccess}) async {
